...
# код обернут в функцию, так как я больше ничего не придумал, чтобы получить словари...
def data_preparation_for_games(give_dicts=False):
    ...
    games_dataset = games_dataset.loc[((games_dataset.category == '0')
                                       ...
                                       | (games_dataset.category == '11')) & (
                                                  games_dataset.release_dates.isnull() == False)
    ...
    if give_dicts:
        return game_id_name, game_name_id

    # так, спокойно, это сделано, так как у меня больше не было идей как завершить процесс
    # если оставить код без exit, то придется долго ждать выполнения
    ...
    dropped_cols = ['aggregated_rating', 'aggregated_rating_count', 'game_engines', 'status', 'release_dates',
                    'age_ratings', 'dlcs', 'franchise', 'parent_game', 'websites', 'external_games',
                    'alternative_names']
